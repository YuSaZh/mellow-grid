...
- **Bento 视觉系统**：用柔和圆角、模块化卡片和紧凑网格承载链接、文字、分栏、活动、音乐、地图和媒体内容。
- **所见即所得**：编辑器复用主页渲染逻辑，尽量减少编辑视图和最终页面之间的差异。
- **个人友好**：本地草稿、JSON 数据源、静态 HTML 导出，适合一个人维护自己的主页。
...
| 路由 | 说明 |
| --- | --- |
| `/` | 由 `data/pages/username.json` 生成 `defaultPageConfig` 后渲染的公开主页。 |
...
- 调整链接卡片颜色、背景、渐变和图标。
- 导入和导出 JSON 配置。
- 保存浏览器会话草稿，并在 `sessionStorage` 不可用时优雅降级。
- 导出独立静态 HTML。
- 公开主页不加载完整编辑器依赖。

## 当前模块

MellowGrid 当前通过 widget registry 支持基础主页模块和更丰富的个人页模块：
...
| `github-activity` | GitHub 资料和贡献活动卡片。 |
| `music` | 播放列表、单曲、专辑或音乐嵌入卡片。 |
| `map` | 带地图呈现和可选嵌入能力的位置卡片。 |
| `media` | 图片、视频或嵌入媒体卡片。 |
...
data/pages/username.json       构建时主页配置源
.github/workflows/deploy-pages.yml  手动触发的 GitHub Pages 部署 workflow
```
...
MellowGrid 当前采用纯静态安全模型：编辑器不会在浏览器里持有 GitHub token，也不会直接写仓库。主页构建数据流是：
...
1. 打开 `/editor`。
2. 调整 profile、链接卡片、文字卡片、分栏符、活动、音乐、地图和媒体 widget。
3. 点击保存，把草稿写入浏览器 `sessionStorage`；即使会话存储被浏览器拦截，编辑器也不会崩溃。
4. 点击导出 JSON，下载得到 `username.json`。
5. 用导出的文件替换仓库里的 `data/pages/username.json`。
6. 提交并推送变更。
7. 在 GitHub Actions 中手动运行 `Deploy GitHub Pages` workflow，构建 `dist/` 并部署到 GitHub Pages。

如果只想单独发布一个 HTML 文件，也可以导出静态 HTML 并直接上传生成的 `index.html`。
...
- [ ] 完善主题 token 和更多预设风格。
- [ ] 继续打磨已有 rich widgets，并只在适合 Bento 主页模型时新增模块。
- [ ] 优化静态 HTML 导出体验。
- [ ] 随行为增长继续补充有针对性的布局和配置规范化测试。
...
- 默认 showcase 页面应覆盖所有已注册 widget 类型，并通过布局不重叠测试保护。
